...
{
  return ((uint16_t)random_uint8(time) << 8) | random_uint8(time);
}

static inline uint32_t random_uint32(size_t time)
{
  return ((uint32_t)random_uint16(time) << 16) | random_uint16(time);
}

static inline uint64_t random_uint64(size_t time)
{
  return ((uint64_t)random_uint32(time) << 32) | random_uint32(time);
}
